...
<body>
		...
		<form action="../project/updateReview.do?review_id=${dto.review_id}" method="post" enctype="multipart/form-data">
		<input type="hidden" name="recipe_id" value="${recipe_id}">
		<h3>레시피 아디 : ${recipe_id }</h3>
...
<!-- 				<span class="star-cb-group"> -->
      ...
      <div class="star-rating__wrap">
        <input class="star-rating__input" id="star-rating-5" type="radio" name="rate" value="5" <c:if test="${dto.review_rating == 5 }">checked</c:if>>
        <label class="star-rating__ico fa fa-star-o fa-lg" for="star-rating-5" ></label>
        <input class="star-rating__input" id="star-rating-4" type="radio" name="rate" value="4"<c:if test="${dto.review_rating == 4 }">checked</c:if>>
        <label class="star-rating__ico fa fa-star-o fa-lg" for="star-rating-4" ></label>
        <input class="star-rating__input" id="star-rating-3" type="radio" name="rate" value="3" <c:if test="${dto.review_rating == 3 }">checked</c:if>>
        <label class="star-rating__ico fa fa-star-o fa-lg" for="star-rating-3" ></label>
        <input class="star-rating__input" id="star-rating-2" type="radio" name="rate" value="2" <c:if test="${dto.review_rating == 2 }">checked</c:if>>
        <label class="star-rating__ico fa fa-star-o fa-lg" for="star-rating-2" ></label>
        <input class="star-rating__input" id="star-rating-1" type="radio" name="rate" value="1" <c:if test="${dto.review_rating == 1 }">checked</c:if>>
...
<!--   				</span> -->
			<hr>
			<pre><textarea name="review_content" style="width: 35%; height: 150px"
				placeholder="리뷰를 입력하세요">${dto.review_content}</textarea></pre>
			<br>
			<br>
			<p>이미지 업로드</p>
			...
			<c:choose>
				...
				<li class="fileItem">
					<div name="review_img[]" class="fileName">${list.img_image_url}</div>
