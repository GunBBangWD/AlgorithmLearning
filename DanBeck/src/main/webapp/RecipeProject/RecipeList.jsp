...
<div>
	<table>
		<tr>
			<th>방법별</th>
			<td><a class="cate_list" href="../project/recipeList.do?page=1">전체</a></td>
			<c:forEach var="cate1" items="${cate1}">
				<td><a class="cate_list" href="../project/recipeList.do?page=1&cate1=${cate1.category_kind_idx}&amp;cate2=${param.cate2}">${cate1.category_kind_name}&nbsp</a></td>
			</c:forEach>
		</tr>
		<tr>
			<th>재료별</th>
			<c:forEach var="cate2" items="${cate2}">
				<td><a class="cate_list" href="../project/recipeList.do?page=1&cate1=${param.cate1}&amp;cate2=${cate2.category_kind_idx}">${cate2.category_kind_name}&nbsp</a></td>
			</c:forEach>
		</tr>
	</table>
</div>
...
<div>		
	...
	<c:if test="${page.cat1==null&&page.cat2==null}">
...
<!-- 페이지 이동 :검색결과 보기 -->
 ...
 <div style="text-align: center;">
	<a class="pagenum" href="?page=1&cate1=${param.cate1}&cate2=${param.cate2}">&lt;&lt;</a>   <!-- 요청url은 동일하고 파라미터만 변경됩니다. -->
	<a class="${page.startPage>1? 'pagenum':'none'}" href="?page=${page.startPage-1 }&cate1=${page.cat1}&cate2=${page.cat2}">&lt;</a>  
	<!-- 현재페이지값을 변경 : blists.getStartPage() -1 -->
	
	<c:forEach var="i" begin="${page.startPage }" end="${page.endPage}">  <!-- 페이지목록의 범위  -->
		<a class="pagenum" href="?page=${i}&cate1=${param.cate1}&cat2=${param.cate2}">${i}</a>     <!-- 현재페이지 i값으로 변경  -->
	</c:forEach>
	
	<!-- 현재페이지값을 변경 : blists.getEndPage() +1 -->
	<a class="${page.endPage!=page.totalPage? 'pagenum':'none'}" href="?page=${page.endPage+1 }&cat1=${page.cat1}&cat2=${page.cat2}">&gt;</a> 
	<a class="pagenum" href="?page=${page.totalPage }&cate1=${param.cate1}&cate2=${param.cate1}">&gt;&gt;</a>
</div>
</c:if>
